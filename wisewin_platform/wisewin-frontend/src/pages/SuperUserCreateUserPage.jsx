// Super user page — form to create a new regular user in their company
// The role is fixed to "User" — super users cannot create other super users or admins
...
export default function SuperUserCreateUserPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      // Role is always "User" — the backend uses the super user's own companyId automatically
      ...
      setForm({ email: "", password: "" }); // clear form so they can create another one
    } catch (err) {
      console.error(err);
  ...
  return (
    <DashboardLayout sidebarItems={sidebarItems} title="Create User">
      <div>
        ...
        <p className="text-[#454652] mt-2">Create a new employee account for your company.</p>
      </div>
